...
#  + reasonable options for the PSM matching

# Sim. #5: ATT for PSM and MDM --------------------
...
setwd("~/PSM_research/simple_dist_matching_paradox/sim5")
...
for(z in 1:n){
  ...
  #     unrelated to Y
  Y <- rnorm(n = 200, mean = ifelse(mydat$a > mean(mydat$a), 5, 10), sd=1) # add to mydat after PS logit
  
  Y[mydat$a > mean(mydat$a)]  <- Y[mydat$a >  mean(mydat$a)] + 2*mydat$treat[mydat$a > mean(mydat$a)]
  Y[mydat$a <= mean(mydat$a)] <- Y[mydat$a <= mean(mydat$a)] + 0.5*mydat$treat[mydat$a <= mean(mydat$a)]
  ...
  for(i in 1:85){
    ...
    many_to_one  <- ifelse(i > 10, 3, 1)
    adjustment   <- ifelse(i > 30, T, F)
    caliper      <- ifelse(i > 30, .05, .1)
    effect       <- Match(Y          = pdat$Y, 
                          ...
                          M          = many_to_one, 
                          replace    = T,
                          BiasAdjust = adjustment,
                          ties       = T,
                          caliper    = caliper # works better than just CommonSupport = T
    )
  ...
  saveRDS(mestmat, paste("saved_objects/sim5_att_mdm_newdata_opts", z, ".rds", sep=""))
  saveRDS(pestmat, paste("saved_objects/sim5_att_psm_newdata_opts", z, ".rds", sep=""))
}
...
for(j in 1:n){
  mymdm <- readRDS(paste("saved_objects/sim5_att_mdm_newdata_opts", j, ".rds", sep=""))
  mypsm <- readRDS(paste("saved_objects/sim5_att_psm_newdata_opts", j, ".rds", sep=""))
...
mae.m <- apply(allMDM, 2, mean)
mae.p <- apply(allPSM, 2, mean)

pdf("../figs/sim5_true_psm_mdm_mae_newdata_opts.pdf")
plot(mae.p, type="l", lwd=2, col="red", 
     ylab="PSM (Options Enabled) vs MDM MAE", 
     xlab="Number of Units Pruned", xaxt="n",ylim=c(min(mae.p,mae.m),
                                                    max(mae.p,mae.m)))
axis(side=1,
     at=c(0, 20, 40, 60, 80),
     labels=c(0, 40, 80, 120, 160))
lines(mae.m, type="l", lwd=2, col="black", lty=1)
