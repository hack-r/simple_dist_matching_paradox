# Same as sim4 but with additional data

# Sim. #4: ATE for PSM and MDM --------------------
...
setwd("/home/serveradmin/PSM_research/variations_on_section5/sim4")
...
for(z in 1:n){
  ...
  saveRDS(mestmat, paste("saved_objects/sim4_ATE_mdm_newdata", z, ".rds", sep=""))
  saveRDS(pestmat, paste("saved_objects/sim4_ATE_psm_newdata", z, ".rds", sep=""))
}
...
# MAE
plot(rev(abs(1-apply(pestmat,2,mean))),col="red",ylim=c(0,1)) #mse
...
for(j in 1:n){
  mymdm <- readRDS(paste("saved_objects/sim4_ATE_mdm_newdata", j, ".rds", sep=""))
  mypsm <- readRDS(paste("saved_objects/sim4_ATE_psm_newdata", j, ".rds", sep=""))
...
pdf("../figs/sim4_true_psm_mdm_ATE_newdata.pdf")
plot(est.p, type="l", lwd=2, col="red", 
     ylab="PSM vs MDM ATE across Pruning Levels", 
...
pdf("../figs/sim4_true_psm_mdm_mae_newdata_ate.pdf")
plot(est.p, type="l", lwd=2, col="red", 
     ylab="PSM vs MDM MAE across Pruning Levels (ATE)", 
     xlab="Number of Units Pruned", xaxt="n") #,ylim=c(0.9,2.3)
axis(side=1,
     at=c(0, 20, 40, 60, 80),
     labels=c(0, 40, 80, 120, 160))
lines(est.m, type="l", lwd=2, col="black", lty=1)
text(x=20, y=0.25, labels="PSM")
text(x=20, y=0.45, labels="MDM")
dev.off()
